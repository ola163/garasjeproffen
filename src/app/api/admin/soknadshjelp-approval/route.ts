...
export async function POST(request: Request) {
  try {
    const { approverEmail, approverName, requesterName, ticketNumber, customerName, totalPrice, soknadshjelId } =
    ...
    const totalFormatted = totalPrice
      ? new Intl.NumberFormat("nb-NO", { style: "currency", currency: "NOK", maximumFractionDigits: 0 }).format(totalPrice)
      : "–";
    ...
    await resend.emails.send({
      ...
      html: `
        <div style="font-family:sans-serif;max-width:560px;margin:0 auto">
          <h2 style="color:#e2520a">Godkjenning av søknadshjelp forespurt</h2>
          <p>Hei ${approverName},</p>
          <p><strong>${requesterName}</strong> ber deg gjennomgå og godkjenne en søknadshjelp-sak.</p>
          ...
          <div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:8px;padding:16px;margin:20px 0">
            <table style="width:100%;font-size:14px;border-collapse:collapse">
              ...
              ${totalPrice ? `
              <tr>
                <td style="padding:4px 0;color:#6b7280">Estimert pris</td>
          ...
          <p style="color:#6b7280;font-size:12px;margin-top:16px">
            Logg inn på admin-panelet for å se detaljer og godkjenne.
          </p>
