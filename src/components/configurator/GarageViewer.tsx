...
function GarageWindowElements({ elements, lengthM, widthM, halfLOverride, halfWOverride, doorWidthM, hasFlatRoof, portOffsetX = 0 }: {
  ...
  const matInterior = useMemo(() => new THREE.MeshStandardMaterial({ color: "#6b6b6b", roughness: 0.85, side: THREE.DoubleSide }), []);
      ...
      if (el.side === "front" || el.side === "back") {
        ...
        if (isGLBWindow) {
          meshes.push(
            ...
            <mesh key={`${key}-int`} position={[x, cy, wFace - dir * (WALL_T + 0.15)]} material={matInterior}>
              <planeGeometry args={[w + FB * 2 + 0.3, h + FB * 2 + 0.3]} />
            </mesh>,
            ...frontRevealsMeshes,
        ...
        if (isGLBWindow) {
          meshes.push(
            ...
            <mesh key={`${key}-int`} position={[wFace - dir * (WALL_T + 0.15), cy, z]} rotation={[0, Math.PI / 2, 0]} material={matInterior}>
              <planeGeometry args={[w + FB * 2 + 0.3, h + FB * 2 + 0.3]} />
            </mesh>,
            ...sideRevealMeshes,
  ...
  return (
    <group position={[portOffsetX, 0, 0]}>
      ...
      <group ref={doorGroupRef}>
        ...
        <mesh position={[targetW / 2 - 0.22, 0.13, doorZ + 0.027]}>
          <planeGeometry args={[0.38, 0.10]} />
          <meshBasicMaterial color="#002F6C" />
        </mesh>
        <Text
          position={[targetW / 2 - 0.22, 0.13, doorZ + 0.028]}
          fontSize={0.062}
          color="#F0A500"
          anchorX="center"
          anchorY="middle"
